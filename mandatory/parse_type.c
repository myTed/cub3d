...
int	parsing_file(t_parse_info *p_parse, char *file_name)
{
	int			read_count;
	int			file_fd;
	int			valid_name;
	...
	if (parse_identifier(file_fd, p_parse, &read_count) == FAIL)
		return (FAIL);
	display_result(p_parse, read_count);	
	return (SUCCESS);
}
...
int	main(int argc, char *argv[])
{
	...
	ft_memset(&parse, 0, sizeof(t_parse_info));	
	ret = parsing_file(&parse, argv[1]);
	return (ret);
}
